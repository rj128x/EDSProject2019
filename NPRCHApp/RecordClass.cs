...
namespace NPRCHApp
{
    ...
    public class RecordHour
    {
        ...
        public void calcReact(List<Record> dataS, bool fillData)
        {
            ...
            List<double> X1avg = new List<double>();
            List<double> Y1avg = new List<double>();
            List<double> X1dx = new List<double>();
            List<double> Y1dy = new List<double>();
            List<double> X1dxAvg = new List<double>();
            List<double> Y1dyAvg = new List<double>();
            ...
            {
                Record rec = dataS[i];
                double pPlan = /*i < dataS.Count - 5 ? dataS[i + 5].P_plan : */rec.P_plan;
                double pZVN = /*i < dataS.Count - 5 ? dataS[i + 5].P_zvn : */rec.P_zvn;
                X.Add((rec.P_perv) / blockData.PNom*100);
                X1.Add((rec.P_perv + pZVN) / blockData.PNom * 100);
                Y.Add((rec.P_fakt - pPlan - pZVN) /blockData.PNom*100);
                Y1.Add((rec.P_fakt - pPlan) / blockData.PNom * 100);
                Xdx.Add(0);
                Ydy.Add(0);
                X1dx.Add(0);
                Y1dy.Add(0);
            ...
            for (int i = 1; i < n; i++)
            {
                ...
                X1dx[i] = X1avg[i] - X1avg[i - 1];
                Y1dy[i] = Y1avg[i] - Y1avg[i - 1];
            }
    ...
    public class Record
    {
        ...
        public Record(DateTime dt, string str,double pNom,double fNom, ref List<double> prevPperv, ref List<double> prevPplan, ref List<double> prevPzvn)
        {
            ...
            /*if (prevPplan.Count > (sdvigP))
                P_plan = prevPplan[prevPplan.Count - sdvigP];

            
            if (prevPzvn.Count > (sdvigP))
                P_zvn = prevPzvn[prevPzvn.Count - sdvigP];*/
                
            
            /*if (prevPzvn.Count > 1)
            {
                double prevZVN = prevPzvn[prevPzvn.Count - 2];
                if (Math.Abs(prevZVN - P_zvn) > stepP)
                {
                    P_zvn = prevZVN + stepP * Math.Sign(P_zvn - prevZVN);
                }
            }
            
            if (prevPplan.Count > 1)
            {
                double prevPlan = prevPplan[prevPplan.Count - 2];
                if (Math.Abs(prevPlan - P_plan) > stepP)
                {
                    P_plan = prevPlan + stepP * Math.Sign(P_plan - prevPlan);
                }
            }*/
